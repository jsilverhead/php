...
2. API возвращает ссылку на загрузку файла (ссылка будет указывать на FUS) формата `example.com/upload/:jwt`
3. Клиент загружает файл по указанной ссылке, используя `multipart/form-data`. Если файл будет соответствовать требованиям, указанным в JWT, то данный микросервис вернёт новый JWT, который нужно будет вернуть в API (мы его записываем обычно в виде `File` или `UploadedRasterImage`)
4. Клиент вызвает метод API для сохранения файла и передаёт в него полученный от микросервиса JWT. На этом этапе API редактирует профиль и обновляет аватар пользователя.
