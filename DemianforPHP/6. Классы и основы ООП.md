...
Трейты
--
Трейт - механизм обеспечения повторного использования кода.
...
trait noNegative
{
    public function checkNegative($array)
    {
...
$newArray = new checkArray;

$array = [-9, 2, 0, 33, 4, -22, -1, 1, 6, 71, 42];

$array = $newArray->checkNegative($array);

$array = $newArray->checkEven($array);

print_r($array); // выдаст [33, 1, 71]
```

Магические методы
--
`__clone` - клонирование объектов в php. Метод `clone` работает именно с клонированным объектом, а не с исходником.
...
**Геттеры и сеттеры**
`__get` - магический метод для получения данных о свойстве класса.
`__set` - магические метод для изменения свойства класса.
